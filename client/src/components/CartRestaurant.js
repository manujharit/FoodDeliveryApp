...
const CartRestaurant = ({ restaurantId, restaurantData }) => {
    ...
    return (
        <div className="border mt-4 border-gray-300" key={restaurantId}>
            <Link to = {`/restaurant/${restaurantId}`} className='px-4 py-2 w-full drop-shadow-lg bg-gray-200 flex flex-row items-start'>
                <div>
                ...
                <div className='flex flex-col items-start justify-start ml-4'>
                    ...
                    <span className=' flex flex-row justify-between w-[100%] text-xs text-black font-semibold'>
                        {restaurantData.info.areaName} <b>&#183;</b>  {restaurantData.info.sla.slaString} <b>&#183;</b> <Location/>  {restaurantData.info.sla.lastMileTravelString}
                    </span>
                </div>
            </Link>
